 ...
 *
 * `wrap/src/fs/home.ts` constructs `wrapFs = createAppFs({ app: "wrap" })` at
 * module load — `wrapFs.root` is captured once and never re-read. Any test
 * that imports a wrap module without WRAP_HOME pre-set would point at the
 * developer's real `~/.wrap`. Setting `process.env.WRAP_HOME` at the top of
 * a test file does NOT work: ES module imports are hoisted, so the wrap
 * import (and therefore wrapFs construction) runs BEFORE the env-var
 * assignment.
 *
