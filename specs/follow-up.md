...
- **Thread the new `risk_level` through to `query.ts` after a follow-up command swap.** Currently `runQuery` gates the dialog on the *original* `response.risk_level` and after the dialog returns it executes using the same value, even if a follow-up swapped in a command with a different risk level. The dialog's internal `setRiskLevel` updates display only — the value never crosses the API boundary. Step 7b's stub never returns `command`, so this is dormant; step 8 wires in real follow-ups and exposes it. Either add `riskLevel` to `DialogResult.run` or have `runQuery` re-read from the swapped response. The log round needs the correct risk_level for accuracy.
