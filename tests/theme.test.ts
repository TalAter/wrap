...
describe("resolveColorHex with ColorRef overrides", () => {
  ...
  test("plain Color tuple resolves at truecolor unchanged", () => {
    ...
    expect(resolveColorHex([245, 186, 74])).toBe("#f5ba4a");
  });

  test("ColorRef base used at truecolor, overrides ignored", () => {
    process.env.FORCE_COLOR = "3";
    expect(
      resolveColorHex({ base: [245, 186, 74], ansi16: [170, 85, 0], ansi256: [200, 100, 0] }),
    ).toBe("#f5ba4a");
  });
  ...
  test("ColorRef ansi16 override applied at level 1", () => {
    ...
    expect(resolveColorHex({ base: [245, 186, 74], ansi16: [170, 85, 0] })).toBe("#aa5500");
  });

  test("ColorRef ansi256 override applied at level 2", () => {
    process.env.FORCE_COLOR = "2";
    // Override [175, 95, 0] hits an exact 256-cube level.
    expect(resolveColorHex({ base: [245, 186, 74], ansi256: [175, 95, 0] })).toBe("#af5f00");
  });

  test("ColorRef falls back to base when matching override missing", () => {
    process.env.FORCE_COLOR = "2";
    // No ansi256 override → base [245,186,74] quantized to 256-cube.
    const baseOnlyAt256 = resolveColorHex({ base: [245, 186, 74], ansi16: [170, 85, 0] });
    const plainAt256 = resolveColorHex([245, 186, 74]);
